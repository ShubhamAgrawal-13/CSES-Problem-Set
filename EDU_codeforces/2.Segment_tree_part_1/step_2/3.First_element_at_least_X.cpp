...
public:
	...
	item find_above(int v, int x, int lx, int rx){
		if(values[x]<v){
			return -1;
		}
		if(rx-lx==1){
		...
		int mid = (lx+rx)/2;
		int res = find_above(v, 2*x+1, lx, mid);
		if(res==-1){
			res = find_above(v, 2*x+2, mid, rx);
		}
		return res;
	}

	item find_above(int v){
		return find_above(v, 0, 0, size);
	}
